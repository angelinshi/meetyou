...
// ── 单条帖子卡片 ─────────────────────────────────────────────────
function DiaryPostCard({ post, showDayTag, isLast, playingId, onPlay, onReply, replyingId }) {
    ...
    <div style={{ display: 'flex', paddingLeft: 16, paddingRight: 16, animation: 'diaryFade 0.22s ease' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, paddingLeft: 10, paddingBottom: isLast ? 12 : 18 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6, paddingTop: showDayTag ? 0 : 1 }}>
          ...
          <span style={{ fontSize: 11, color: MY.textTer, marginLeft: 'auto' }}>{post.time}</span>
        </div>
        ...
        <div style={{ marginTop: 6 }}>
          ...
          <div onClick={() => onReply && onReply(post.id)} style={{
            marginTop: (post.comments && post.comments.length) ? 5 : 2,
            paddingBottom: 4, cursor: 'text', transition: 'border-color 0.15s',
            borderBottom: `1px solid ${isReplying ? MY.brandRed : MY.line}`,
          }}>
            <span style={{ fontSize: 12, color: MY.textTer }}>说点什么…</span>
          </div>
        </div>
      </div>
    </div>
...
function DiaryScreenInner({ onBack, onPublish }) {
  ...
  React.useEffect(() => {
    ...
    if (!el) return;
    const fn = () => setNav(el.scrollTop > 170);
...
function DiaryHomeModule({ onViewAll }) {
  return (
    // padding-bottom 显式设值，避免最后子元素 margin 折叠导致底部无间隙
    <div style={{ background: MY.surface, borderRadius: MY.rmd, padding: '14px 14px 14px', boxShadow: MY.shadow1 }}>
            ...
            return (
              <div key={post.id} style={{ display: 'flex' }}>
                ...
                <div style={{ width: 18, flexShrink: 0, display: 'flex', flexDirection: 'column',
                  ...
                  {!prevSameDay && (
                    <div style={{ width: 9, height: 9, borderRadius: '50%', background: MY.brandRed,
                      flexShrink: 0, zIndex: 1 }}/>
                  )}
                  {!isLast && (
                    <div style={{ flex: 1, width: 1.5, background: '#E8E8E8',
                      marginTop: !prevSameDay ? 3 : 0 }}/>
                  )}
                </div>
                {/* 内容 */}
                <div style={{ flex: 1, minWidth: 0, paddingLeft: 8, paddingBottom: isLast ? 0 : 16 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 10, color: MY.textTer, marginLeft: 'auto' }}>{post.time}</span>
                  </div>
                  {/* 内容气泡：上文下图 */}
      ...
      {/* 按钮：上 10px 间距，padding-bottom 已在容器上保证底部呼吸 */}
...
function Screen5DiaryAll({ onBack, onPublish }) {
  return (
    <Phone bg={MY.surface} statusDark>
      <DiaryScreenInner onBack={onBack} onPublish={onPublish}/>
    </Phone>
  );
}
...
function BFDiary({ onBack }) {
  return (
    <Phone bg={MY.surface} statusDark>
      <DiaryScreenInner onBack={onBack} onPublish={() => {}}/>
    </Phone>
  );
}
